...
def load_bert_encoding(encoding_path):
    """Load the Bert encoding of each document

    Args:
        encoding_path: Path to the encoding file

    Returns:
        matrix: The encoding of the dataset
    """
    with bz2.open(encoding_path, 'rt', newline='', encoding='utf8') as tsv_file:
        reader = csv.reader(tsv_file, delimiter='\t')
        matrix = []
